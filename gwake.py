...
class ShellController():
  def __init__(self, config_parser):
    ...
    self.shell_executable = self.config_parser.get('Gwake', 'ShellExecutable')
    self.shell_font = self.config_parser.get('Gwake', 'ShellFont')
    self.cygwin_path = self.config_parser.get('Gwake', 'CygwinPath')
    mywindow_broadcaster_cygwin_path = '/cygdrive/' + os.path.abspath('broadcast-mywindow.sh').replace('\\', '/').replace(':', '', 1)
    self.shell_open_command = [os.path.join(self.cygwin_path, 'bin', self.shell_executable), '-sl', '1500', '-fn', 
                               self.shell_font, '-bg', 'black', '-fg', 'grey', '-sr', '-e',
                               './bash.exe', '--rcfile', mywindow_broadcaster_cygwin_path]
    self.transparent_factor = self.config_parser.getfloat('Gwake', 'ShellTransparency')
    self.shell_height = self.config_parser.getint('Gwake', 'ShellHeight')
  ...
  def send_string_keystrokes(self, str):
    for char in str:
      win32gui.SendMessage(self.shell_hwnd, win32con.WM_CHAR, ord(char), 0)
